---
title: Railway
description: Track memory, CPU, network, and disk across all projects
---
...
- Shortly after a billing period starts, values may appear lower than expected because there are fewer minutes of data to average over
...
**No data / all zeros**
If all your Railway services are idle (sleeping or no traffic), the usage API may return zero values. This is expected — Stackwatch will still show the integration as connected. Values will update once your services are active.

**Missing projects**
Stackwatch queries all projects accessible to the token. If a project is missing, check that the Railway account connected is the owner (not just a collaborator) of that project.
...
2. In Stackwatch, go to **Integrations**, find your Railway integration, and click the **edit (pencil)** icon
